...
struct SUlamek
{
	...
	SUlamek operator*(SUlamek);
};

SUlamek SUlamek::operator*(SUlamek u2)
{
	SUlamek result;
	result.licznik = this->licznik * u2.licznik;
	result.mianownik = this->mianownik * u2.mianownik;

	return result;
}

SUlamek Skracja(SUlamek u)
{
	bool flagEnd = false;
	...
	while (flagEnd == false)
	{
		if (i <= u.licznik && i <= u.mianownik)
		{
			if ((u.licznik%i == 0) && (u.mianownik%i == 0))
			{
				u.licznik = u.licznik / i;
				u.mianownik = u.mianownik / i;
				i = 2;
			}
			else
			{
...
void zad_3(void)
{
	vector<SUlamek> tablicaUlamkow;

	cout << "Podaj ulamki:" << endl;
	for (int i = 0; i < 2; ++i)
	{
		SUlamek nowyUlamek;
		cin >> nowyUlamek.licznik;
		cin >> nowyUlamek.mianownik;
		tablicaUlamkow.push_back(nowyUlamek);
	}

	SUlamek iloczynUlamkow;
	iloczynUlamkow.licznik = 1;
	iloczynUlamkow.mianownik = 1;
	for each (auto var in tablicaUlamkow)
	{
		iloczynUlamkow = iloczynUlamkow * var;
	}

	cout << iloczynUlamkow.licznik << "/" << iloczynUlamkow.mianownik << endl;

	SUlamek skroconyUlamek = Skracja(iloczynUlamkow);

	cout << skroconyUlamek.licznik << "/" << skroconyUlamek.mianownik << endl;
